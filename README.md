...
uv run uvicorn vorrat_classifier.main:app --reload --port 8001
...
VORRAT_CLASSIFIER_MODEL_PATH="$(pwd)/models/Qwen2.5-7B-Instruct-Q4_K_M.gguf" \
  uv run uvicorn vorrat_classifier.main:app --port 8001
```
...
services:
  classifier:
    image: ghcr.io/idleherb/vorrat-classifier:stable-latest
    restart: unless-stopped
    volumes:
      - ./models:/models:ro
    environment:
      VORRAT_CLASSIFIER_MODEL_PATH: /models/Qwen2.5-7B-Instruct-Q4_K_M.gguf
      VORRAT_CLASSIFIER_N_THREADS: "4"
    labels:
...
Every setting is `VORRAT_CLASSIFIER_<UPPER_SNAKE>` in the environment.
